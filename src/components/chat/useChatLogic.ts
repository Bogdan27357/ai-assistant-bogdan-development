  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (errorMessage.includes('Rate limit exceeded') || errorMessage.includes('free-models-per-day')) {
        userFriendlyMessage = '⚠️ Превышен лимит бесплатных запросов OpenRouter на сегодня.\n\n' +
          '💡 Решения:\n' +
          '• Попробуйте другую модель в меню сверху\n' +
          '• Добавьте кредиты на openrouter.ai\n' +
          '• Подождите до завтра для сброса лимита';
        toast.error('Лимит бесплатных запросов исчерпан');
      } else if (errorMessage.includes('Failed to fetch') || errorMessage.includes('API ключи')) {
